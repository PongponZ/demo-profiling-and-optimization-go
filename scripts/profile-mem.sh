...
# Usage: ./scripts/profile-mem.sh [bad|optimized] [duration]
...
echo "Starting memory profiling for $VERSION worker..."
...
"$WORKER_BIN" \
    ...
    -data-size=200 \
    -duration="$DURATION" \
    -memprofile="$OUTPUT_FILE" 2>&1 | tee "$OUTPUT_DIR/mem-${VERSION}-output.log" || true
...
echo ""
echo "To see top memory allocations:"
echo "  go tool pprof -top $OUTPUT_FILE"
